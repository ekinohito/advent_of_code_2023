...
impl Grid {
    pub fn new(matrix: Vec<Vec<bool>>) -> Self {
        let n = matrix.len();
        let m = matrix.first().expect("Grid must contain at least one row").len();
        Self { matrix, n, m }
    }
    ...
    fn is_v_symmetric(&self, i: usize) -> bool {
        ...
        left_side.iter().rev().zip(right_side.iter()).all(|(left, right)| left == right)
    }
    ...
    pub fn find_symmetry(&self) -> Option<usize> {
        self.find_v_symmetry().map(|i| 100 * i).or_else(|| self.find_h_symmetry())
    }
...
pub fn solution(input: &str) -> u64 {
    ...
    // dbg!(&grids);
    grids.iter().map(Grid::find_symmetry).map(Option::unwrap).sum::<usize>() as u64
}
